...
package com.example.bottomsheeterrorreportingreproduction.androidapp.media.common
...
import com.example.bottomsheeterrorreportingreproduction.androidapp.media.common.MusicServiceConnection.MediaBrowserConnectionCallback
import com.example.bottomsheeterrorreportingreproduction.androidapp.media.media.NETWORK_FAILURE
...
//        .apply { postValue(NOTHING_PLAYING) }
    ...
    fun setPlaybackSpeed(speed: Float) {
        ld("Setting speed: $speed")
        val command = Player.COMMAND_SET_SPEED_AND_PITCH.toString()
        sendCommand(command, bundleOf(command to speed))
    }
    fun setSkipSilence(shouldSkip: Boolean) {
        ld("Setting skip silence: $shouldSkip")
        sendCommand(
            CONSTANTS.COMMAND_SET_SKIP_SILENCE,
        bundleOf(
            CONSTANTS.COMMAND_SET_SKIP_SILENCE to shouldSkip
        ))
    }
    ...
    private inner class MediaControllerCallback : MediaControllerCompat.Callback() {
        ...
        override fun onMetadataChanged(metadata: MediaMetadataCompat?) {
            ...
            // anything.
            ld("onMetadataChanged(metadata=$metadata (bundle is ${metadata?.bundle?.contentsToString()})")
            if (metadata?.id != null) nowPlaying.postValue(metadata!!)
            else ld("MEDIA_ID was null")
        }

        override fun onQueueChanged(queue: MutableList<MediaSessionCompat.QueueItem>?) {
            ld("onQueueChanged(queue=$queue)")
        }

        override fun onSessionEvent(event: String?, extras: Bundle?) {
            super.onSessionEvent(event, extras)
            ld("onSessionEvent(event=$event, extras=${extras?.contentsToString()})")
            when (event) {
