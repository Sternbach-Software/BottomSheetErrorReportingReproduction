...
import com.example.bottomsheeterrorreportingreproduction.androidapp.media.media.extensions.albumArtist
import com.example.bottomsheeterrorreportingreproduction.androidapp.media.media.extensions.artist
import com.example.bottomsheeterrorreportingreproduction.androidapp.media.media.extensions.containsCaseInsensitive
import com.example.bottomsheeterrorreportingreproduction.androidapp.media.media.extensions.genre
    ...
    override fun search(query: String, extras: Bundle): List<MediaMetadataCompat> {
        ...
        val focusSearchResult = when (extras[MediaStore.EXTRA_MEDIA_FOCUS]) {
            ...
            MediaStore.Audio.Artists.ENTRY_CONTENT_TYPE -> {
                ...
                ld("Focused artist search: '$artist'")
                filter { song ->
            ...
            MediaStore.Audio.Albums.ENTRY_CONTENT_TYPE -> {
                ...
                ld("Focused album search: album='$album' artist='$artist")
                filter { song ->
            ...
            MediaStore.Audio.Media.ENTRY_CONTENT_TYPE -> {
                ...
                val album = extras[MediaStore.EXTRA_MEDIA_ALBUM]
                val artist = extras[MediaStore.EXTRA_MEDIA_ARTIST]
                ld("Focused media search: title='$title' album='$album' artist='$artist")
                filter { song ->
                    (song.artist == artist || song.albumArtist == artist) && song.album == album
                            && song.title == title
                }
        ...
        if (focusSearchResult.isEmpty()) {
            return if (query.isNotBlank()) {
                ...
                ld("Unfocused search without keyword")
                return shuffled()
            }
        } else {
