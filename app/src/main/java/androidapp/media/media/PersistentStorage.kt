...
import com.example.bottomsheeterrorreportingreproduction.androidapp.media.media.extensions.asContentUri
import com.example.bottomsheeterrorreportingreproduction.androidapp.preferencesManager
...
internal class PersistentStorage private constructor(val context: Context) {
    ...
    fun loadRecentSong(): MediaBrowserCompat.MediaItem? {
        val mediaId = preferencesManager.getString(RECENT_SONG_MEDIA_ID_KEY, null)
        return if (mediaId == null) {
            null
        } else {
            val extras = Bundle().also {
                val position = preferencesManager.getLong(RECENT_SONG_POSITION_KEY, 0L)
            ...
            MediaBrowserCompat.MediaItem(
                MediaDescriptionCompat.Builder()
                    .setMediaId(mediaId)
                    .setTitle(preferencesManager.getString(RECENT_SONG_TITLE_KEY, ""))
                    .setSubtitle(preferencesManager.getString(RECENT_SONG_SUBTITLE_KEY, ""))
                    .setIconUri(Uri.parse(preferencesManager.getString(RECENT_SONG_ICON_URI_KEY, "")))
                    .setExtras(extras)
                    .build(), FLAG_PLAYABLE
            )
        }
