...
package com.example.bottomsheeterrorreportingreproduction.androidapp.media.media.extensions
...
fun MediaMetadataCompat.toMediaItemMetadata(): com.google.android.exoplayer2.MediaMetadata {
    return with(MediaMetadata.Builder()) {
        ...
        val extras = Bundle()
        getString(ShiurQueue.ORIGINAL_ARTWORK_URI_KEY)?.let {
            // album art is a content:// URI. Keep the original for Cast.
            extras.putString(
                ShiurQueue.ORIGINAL_ARTWORK_URI_KEY,
                getString(ShiurQueue.ORIGINAL_ARTWORK_URI_KEY)
            )
        }
...
fun MediaMetadataCompat.toMediaItem(playbackProgress: Long): com.google.android.exoplayer2.MediaItem {
    ld("Playback progress: $playbackProgress")
    return with(com.google.android.exoplayer2.MediaItem.Builder()) {
        setMediaId(mediaUri.toString())
        setUri(mediaUri)
        setMimeType(MimeTypes.AUDIO_MPEG)
        setClippingConfiguration(com.google.android.exoplayer2.MediaItem.ClippingConfiguration.Builder().setStartPositionMs(playbackProgress).build()) //TODO using getNormalizedShiurProgressOffset is probably the wrong way to solve this problem, because it means duration is not getting updated correctly, and that may be the source of the progress fill flicker
 ...
 */
const val METADATA_KEY_UAMP_FLAGS = "com.example.bottomsheeterrorreportingreproduction.androidapp.media.METADATA_KEY_UAMP_FLAGS"
